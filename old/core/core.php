...
if (!is_dir($_SESSION['private_folder'])) {
    mkdir($_SESSION['private_folder'], 0744);
}
if (!is_file($_SESSION['config_file'])) {
    $_SESSION['config'] = extract_config_vars();
    save_config($_SESSION['config']);
} elseif (empty($_SESSION['config'])) {
    $_SESSION['config'] = load_config();
}
...
#################################################
if (!empty($_GET)) {
    $_GET = array_map('deep_strip_tags', $_GET);
}
if (!empty($_POST)) {
    $_POST = array_map('deep_strip_tags', $_POST);
}
if (!empty($_SESSION)) {
    $_SESSION = array_map('deep_strip_tags', $_SESSION);
}
if (!empty($_FILES)) {
    $_FILES = array_map('deep_strip_tags', $_FILES);
}
#################################################

# locale
if (!conf('language')) {
    conf('language', $default_language);
}
if (is_file('locale/' . conf('language') . '.php')) {
    include('locale/' . conf('language') . '.php');
} else {
    $lang = array();
}
# Aspect config
if (!conf('aspect')) {
    conf('aspect', $default_aspect);
}
if (!conf('theme')) {
    conf('theme', $default_theme);
}
if (!conf('mode')) {
...
if (!conf('stats_max_entries')) {
    conf('stats_max_entries', $default_limit_stat_file_entries);
}
if (!conf('stats_max_lines')) {
    conf('stats_max_lines', $default_max_lines_per_page_on_stats_page);
}
if (empty($_SESSION['private_folder'])) {
    $_SESSION['private_folder'] = $default_private;
}
if (empty($_SESSION['zip'])) {
    $_SESSION['zip'] = class_exists('ZipArchive');
}
if (empty($_SESSION['curl'])) {
    $_SESSION['curl'] = function_exists('curl_init');
}
if (empty($_SESSION['GD'])) {
    $_SESSION['GD'] = function_exists('imagecreatetruecolor');
}
if (empty($_SESSION['home'])) {
    $_SESSION['home'] = getRacine();
}
if (empty($_SESSION['clean_temp_folder_time'])) {
    $_SESSION['clean_temp_folder_time'] = $clean_temp_folder_time;
}
if (empty($_SESSION['temp_folder'])) {
    $_SESSION['temp_folder'] = $default_temp_folder;
}
if (empty($_SESSION['root'])) {
    $_SESSION['root'] = ROOT;
}
if (empty($_SESSION['id_file'])) {
    $_SESSION['id_file'] = $default_id_file;
}
if (empty($_SESSION['folder_share_file'])) {
    $_SESSION['folder_share_file'] = $default_folder_share_file;
}
if (empty($_SESSION['stats_file'])) {
    $_SESSION['stats_file'] = $default_stat_file;
}
if (!conf('max_files_per_page')) {
    conf('max_files_per_page', $default_max_files_per_page);
}
if (!isset($_SESSION['current_path'])) {
    $_SESSION['current_path'] = "";
}
if (!isset($_SESSION['users_rights_file'])) {
    $_SESSION['users_rights_file'] = $default_users_rights_file;
}
...
if (empty($_SESSION['profiles_rights_file'])) {
    $_SESSION['profiles_rights_file'] = $default_profiles_rights_file;
}
...
if (!isset($_SESSION['temp_cleaned']) || time() > $_SESSION['temp_cleaned'] + $_SESSION['clean_temp_folder_time']) {
    ...
    foreach ($files as $file) {
        if (time() > filectime($file) + $_SESSION['clean_temp_folder_time']) {
            unlink($file);
        }
    }
...
if (!is_writable($_SESSION['private_folder'])) {
    echo '<p class="error">' . e('Private folder is not writable', false) . '</p>';
}
if (!is_readable($_SESSION['private_folder'])) {
    echo '<p class="error">' . e('Private folder is not readable', false) . '</p>';
}
if (!is_writable($_SESSION['temp_folder'])) {
    echo '<p class="error">' . e('Temp folder is not writable', false) . '</p>';
}
if (!is_readable($_SESSION['temp_folder'])) {
    echo '<p class="error">' . e('Temp folder is not readable', false) . '</p>';
}
if (!is_readable($_SESSION['private_folder'] . 'trees')) {
    $message .= '<div class="error">' . e('Problem accessing tree folder: not readable', false) . '</label>';
}
if (!is_writable($_SESSION['private_folder'] . 'trees')) {
    $message .= '<div class="error">' . e('Problem accessing tree/folder file: not writable', false) . '</label>';
}
if (!empty($_SESSION['upload_user_path']) && !is_readable($_SESSION['upload_root_path'] . $_SESSION['upload_user_path'] . $_SESSION['current_path'])) {
    $message .= '<div class="error">' . e('Problem accessing ', false) . $_SESSION['current_path'] . e(': folder not readable', false) . '</label>';
}
if (!empty($_SESSION['upload_user_path']) && !is_writable($_SESSION['upload_root_path'] . $_SESSION['upload_user_path'] . $_SESSION['current_path'])) {
    $message .= '<div class="error">' . e('Problem accessing ', false) . $_SESSION['current_path'] . e(': folder not writable', false) . '</label>';
}

# Check necessary libs
if (!$disable_non_installed_libs_warning) {
    if (!$_SESSION['zip']) {
        $message .= '<div class="error">ZipArchive ' . e('is not installed on this server', false) . ' <a   href="http://php.net/manual/' . conf('language') . '/zip.setup.php">' . e('More info', false) . '</a></div>';
    }
    if (!$_SESSION['GD']) {
        $message .= '<div class="error">GD ' . e('is not installed on this server', false) . ' <a   href="http://php.net/manual/' . conf('language') . '/image.installation.php">' . e('More info', false) . '</a></div>';
    }
    if (!$_SESSION['curl']) {
        $message .= '<div class="error">Curl ' . e('is not installed on this server', false) . ' <a   href="http://php.net/manual/' . conf('language') . '/curl.setup.php">' . e('More info', false) . '</a></div>';
    }
}
# Check files
...
if (!is_file($_SESSION['private_folder'] . 'index.html')) {
    file_put_contents($_SESSION['private_folder'] . 'index.html', ' ');
}
...
if (!is_file($_SESSION['users_rights_file'])) {
    ...
    $_SESSION['users_rights'] = array();
} else {
    $_SESSION['users_rights'] = load_users_rights();
}
if (!is_file($_SESSION['profiles_rights_file'])) {
    save($_SESSION['profiles_rights_file'], array());
}
if (is_file($_SESSION['id_file']) && !is_readable($_SESSION['id_file'])) {
    $message .= '<div class="error">' . e('Problem accessing ID file: not readable', false) . '</label>';
}
if (is_file($_SESSION['id_file']) && !is_writable($_SESSION['id_file'])) {
    $message .= '<div class="error">' . e('Problem accessing ID file: not writable', false) . '</label>';
}
if (is_file($_SESSION['stats_file']) && !is_readable($_SESSION['stats_file'])) {
    $message .= '<div class="error">' . e('Problem accessing stats file: not readable', false) . '</label>';
}
if (is_file($_SESSION['stats_file']) && !is_writable($_SESSION['stats_file'])) {
    $message .= '<div class="error">' . e('Problem accessing stats file: not writable', false) . '</label>';
}
...
if (empty($RIGHTS)) { #default profiles if not configured
    ...
    $PROFILES = array_filter(array_keys($RIGHTS));
}
...
function store($ids = null)
{
    ...
    natcasesort($ids);
    return save($_SESSION['id_file'], $ids);
}

function unstore()
{
    return array_filter(load($_SESSION['id_file']));
}

function save_folder_share($array = null)
{
    return save($_SESSION['folder_share_file'], $array);
}

function load_folder_share()
{
    return load($_SESSION['folder_share_file']);
}

function save_users_rights($array = null)
{
    return save($_SESSION['users_rights_file'], $array);
}

function load_users_rights()
{
    return load($_SESSION['users_rights_file']);
}

function save_config($array = null)
{
    if (!$array) {
        $array = $_SESSION['config'];
    }
    return save($_SESSION['config_file'], $array);
}

function load_config()
{
    return load($_SESSION['config_file']);
}
...
function delete_file_or_folder($id = null, $ids = null, $tree = array())
{
    ...
    if (empty($tree)) {
        tree(null, $_SESSION['login'], false, false, $tree);
    }
    $f = id2file($id);
    if (is_file($f)) {
        # delete file & thumb
...
{
    if (substr($chaine, strlen($chaine) - 1, 1) != '/' && !empty($chaine)) {
        return $chaine . '/';
    } else {
        return $chaine;
    }
...
function rename_item($file = null, $folder = '')
{
    ...
    while (is_file($destination) || is_dir($destination)) {
        ...
        if (is_file($destination)) {
            $file2 = str_replace('.' . $extension, $add . '.' . $extension, $file);
        } else {
            $file2 = $file . $add;
        }
...
{
    if (empty($tree) || empty($ext)) {
        return false;
    }
    if (is_string($tree)) {
        $extension = pathinfo($tree, PATHINFO_EXTENSION);
        if (stripos($ext, $extension) === false || empty($extension)) {
            return false;
        }
        return true;
...
function burned($id)
{
    if ($id[0] == '*' && !isset($_GET['thumbs'])) {
        if (!is_user_connected() || !is_owner($id)) {
            removeID($id);
        }
    }
}
...
function addID($path, $ids = null)
{# $path must be complete upload path
    global $ids;
    if (!$ids) {
        $ids = unstore();
    }
    if ($id = array_search($path, $ids)) {
        return $id;
    }
    $item_id = uniqid(true);
    $ids[$item_id] = $path;
...
function deep_strip_tags($var)
{
    ...
    if (is_array($var)) {
        return array_map('deep_strip_tags', $var);
    }
    return $var;
}
...
{
    if (!empty($pw)) {
        return hash('sha512', BOZON_SALT . $pw);
    }
    return false;
}
...
function count_back($previous, $current)
{
    ...
    for ($c = 0; $c < $max; $c++) {
        if (!isset($previous[$c]) || !isset($current[$c]) || $previous[$c] != $current[$c]) {
            break;
        }
    }
    $c++;
...
function draw_tree($tree = null)
{
    ...
    if ($readme && !empty($tree[$readme]) && is_file($tree[$readme])) {
        $readme = file_get_contents($tree[$readme]);
    }
    if (!$image_only && !$sound_only) {
        ...
        if (!empty($readme)) {
            echo '<div class="markdown">' . parse($readme) . '</div>';
        }
...
function make_connect_link($label_admin = '&nbsp;', $label_logout = '&nbsp;', $label_login = '&nbsp;')
{
    ...
    if (is_user_connected()) {
        ...
        echo '<a id="login_button" class="btn" href="index.php?p=login" title="' . e('Connection', false) . '"><span class="icon-lock" ></span> ' . $label_login . '</a>';
    }
}
...
function body_classes()
{
    ...
    if (!empty($_GET['p'])) {
        echo preg_replace('#[^a-zA-Z0-9_]#', '', $_GET['p']) . ' ';
    } else {
        echo 'home ';
    }
    if (conf('language')) {
        echo 'body_' . conf('language') . ' ';
    }
    if (conf('mode')) {
        echo conf('mode') . ' ';
    }
    if (conf('aspect') && empty($_GET['f'])) {
        echo conf('aspect') . ' ';
    }
}
...
{
    if (!$id || empty($_SESSION['login']) || !$f = id2file($id)) {
        return false;
    }
    $file = explode('/', $f);
    $owner = $file[1];
...
{
    if (!empty($_SESSION['status']) && $_SESSION['status'] == 'superadmin') {
        return true;
    }
    return false;
}
...
function load_users_list()
{
    ...
    if (empty($auto_restrict["users"])) {
        ...
        if (!is_file($auto_restrict_users)) {
            return false;
        }
...
function complete_users_rights()
{
    ...
    if (!isset($_SESSION['users_rights'])) {
        $_SESSION['users_rights'] = load_users_rights();
    }
...
{
    $tree = tree($folder, $user, false, true);
    $size = 0;
...
{
    if (empty($user) && empty($_SESSION['profile_folder_max_size'])) {
        return false;
    }
    if (empty($user)) {
        return $_SESSION['profile_folder_max_size'];
    }
    if (!empty($_SESSION['users_rights'][$user])) {
        return $_SESSION['users_rights'][$user];
    }
    return false;
}
...
{
    if (!$key && $value === null && empty($_SESSION['config'])) {
        return false;
    }
    if (!$key && $value === null) {
        return $_SESSION['config'];
    }
    if (!isset($_SESSION['config'][$key]) && $value === null) {
        return false;
    }
    if ($value === null) {
...
{
    if (empty($vars) && empty($_SESSION['config'])) {
        $vars = extract_config_vars();
    }
    if (empty($vars)) {
...
function aff($var, $stop = true)
{
    ...
    echo '<pre style="background-color:rgba(0,0,0,0.8);color:#fff;padding:10px"><code>';
    var_dump($var);
    echo '</code></pre>';
...
{
    file_put_contents('debug.html', $str, FILE_APPEND);
}
...
function chrono($name = 'chrono', $delete = false)
{
    ...
    if (!isset($_SESSION[$name])) {
        ...
        $debug = '<li> ' . date('h:i:s') . ' ' . $name . ':<em style="padding:5px;font-size:1.3em; color:' . $color2 . ';background-color:' . $color . '">' . $count . 'sec</em> <ul style="margin-left:30px;">';
        unset($_SESSION[$name]);
        $dat = debug_backtrace();
        foreach (array_reverse($dat) as $data) {
            $dir = dirname($data['file']) . '/';
